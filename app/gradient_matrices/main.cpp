...
void print_usage(const char* prog_name) {
    ...
    std::cout << "  output.mtx  - Output gradient matrix\n";
...
int main(int argc, char* argv[]) {
    ...
    auto G = directional::conf_gradient_matrix_2D<double>(mesh, intrinsic, N);
    write_sparse_matrix(output_file, G);

    std::cout << "Gradient matrix written: " << G.rows() << "x" << G.cols() << "\n";
    std::cout << "Done!\n";
    return 0;
}