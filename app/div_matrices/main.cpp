...
void print_usage(const char* prog_name) {
    ...
    std::cout << "  output.mtx  - Output divergence matrix\n";
...
int main(int argc, char* argv[]) {
    ...
    auto D = directional::div_matrix_2D<double>(mesh, intrinsic, N);
    write_sparse_matrix(output_file, D);

    std::cout << "Divergence matrix written: " << D.rows() << "x" << D.cols() << "\n";
    std::cout << "Done!\n";
    return 0;
}